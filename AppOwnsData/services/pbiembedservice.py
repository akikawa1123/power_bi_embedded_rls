...
from models.effectiveidentity import EffectiveIdentity
...
class PbiEmbedService:
    ...
    def get_embed_params_for_single_report_with_rls(self, workspace_id, report_id, username, roles, additional_dataset_id=None):
        '''Get embed params for a report with Row-Level Security (RLS)

        Args:
            workspace_id (str): Workspace Id
            report_id (str): Report Id
            username (str): Username for RLS (typically customer ID or email)
            roles (list): List of role names defined in Power BI dataset
            additional_dataset_id (str, optional): Dataset Id different than the one bound to the report. Defaults to None.

        Returns:
            EmbedConfig: Embed token with RLS and Embed URL
        '''
        ...
        embed_token = self.get_embed_token_for_single_report_with_rls(report_id, dataset_ids, workspace_id, username, roles)
        embed_config = EmbedConfig(embed_token.tokenId, embed_token.token, embed_token.tokenExpiry, [report.__dict__])
        return json.dumps(embed_config.__dict__)
    ...
    def get_embed_token_for_single_report_with_rls(self, report_id, dataset_ids, target_workspace_id, username, roles):
        '''Get Embed token for single report with Row-Level Security (RLS)

        Args:
            report_id (str): Report Id
            dataset_ids (list): Dataset Ids
            target_workspace_id (str): Workspace Id
            username (str): Username for RLS (typically customer ID or email)
            roles (list): List of role names defined in Power BI dataset

        Returns:
            EmbedToken: Embed token with RLS applied
        '''
        ...
        effective_identity = EffectiveIdentity(username, roles, dataset_ids)
        request_body.identities.append(effective_identity.__dict__)

        # Generate Embed token with RLS. Refer https://learn.microsoft.com/power-bi/developer/embedded/embed-sample-for-customers
        embed_token_api = 'https://api.powerbi.com/v1.0/myorg/GenerateToken'
        api_response = requests.post(embed_token_api, data=json.dumps(request_body.__dict__), headers=self.get_request_header())

        if api_response.status_code != 200:
            abort(api_response.status_code, description=f'Error while retrieving Embed token with RLS\n{api_response.reason}:\t{api_response.text}\nRequestId:\t{api_response.headers.get("RequestId")}')
