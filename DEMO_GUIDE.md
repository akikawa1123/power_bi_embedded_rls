...
```
[ステップ1] Microsoft Entra IDアプリ登録（5分）
      ↓
...
| Microsoft Entra IDへのアクセス | アプリ登録の権限が必要 |
...
## ステップ1: Microsoft Entra ID アプリの登録
...
2. 右上の歯車アイコン → **管理ポータル**
3. **テナントの設定** → **開発者設定**
4. **サービス プリンシパルは Fabric の公開用 API を呼び出すことができます** → **有効**
5. **変更を適用** をクリック
...
| 3 | Server Rack | Furniture | 5000 | customer_b |
| 4 | Mouse | Furniture | 50 | customer_b |
| 5 | Adobe CC | Software | 3000 | customer_c |
| 6 | Router | Furniture | 800 | customer_c |
...
6. `customer_b` でも同様にテスト → **Furniture のみ** 表示を確認 ✅
...
2. **ユーザーまたはグループを追加**
3. Microsoft Entra ID で登録したアプリ名（`PowerBI-Embedded-Demo`）を検索して追加
...
### `Unable to get authority configuration for .../YOUR_TENANT_ID`
→ `config.py` の `TENANT_ID` がプレースホルダーのまま（未設定）になっている  
→ ステップ1-1でメモした**ディレクトリ（テナント）ID**を正しく設定すること
...
// ✅ 正しい
IF(USERNAME() = "customer_a", [Product category] = "Software", TRUE())
```
...
[Python/Flask サーバー]
  ...
  ↓ Microsoft Entra ID から Bearer Token を取得（MSAL）
